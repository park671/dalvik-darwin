...
#include "Log.h"
#include "../libcutils/process_name.h"
...
static int extractAndProcessZip(int zipFd, int cacheFd,
    ...
    if (dexoptFlagStr[0] != '\0') {
        ...
        if (opc != NULL) {
            switch (*(opc + 2)) {
                case 'n': verifyMode = VERIFY_MODE_NONE;
                    break;
                case 'r': verifyMode = VERIFY_MODE_REMOTE;
                    break;
                case 'a': verifyMode = VERIFY_MODE_ALL;
                    break;
                default: break;
            }
        }
        ...
        if (opc != NULL) {
            switch (*(opc + 2)) {
                case 'n': dexOptMode = OPTIMIZE_MODE_NONE;
                    break;
                case 'v': dexOptMode = OPTIMIZE_MODE_VERIFIED;
                    break;
                case 'a': dexOptMode = OPTIMIZE_MODE_ALL;
                    break;
                default: break;
            }
        }
