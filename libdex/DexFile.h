...
extern inline __attribute__((gnu_inline, always_inline)) const DexMapList *dexGetMap(const DexFile *pDexFile) {
...
extern inline __attribute__((gnu_inline, always_inline)) const char *dexGetStringData(const DexFile *pDexFile,
...
extern inline __attribute__((gnu_inline, always_inline)) const DexStringId *dexGetStringId(const DexFile *pDexFile, u4 idx) {
    assert(idx < pDexFile->pHeader->stringIdsSize);
    return &pDexFile->pStringIds[idx];
}

/* return the UTF-8 encoded string with the specified string_id index */
extern inline __attribute__((gnu_inline, always_inline)) const char *dexStringById(const DexFile *pDexFile, u4 idx)  {
...
extern inline __attribute__((gnu_inline, always_inline)) const DexTypeId *dexGetTypeId(const DexFile *pDexFile, u4 idx) {
 ...
 */
extern inline __attribute__((gnu_inline, always_inline)) const char *dexStringByTypeIdx(const DexFile *pDexFile, u4 idx) {
...
extern inline __attribute__((gnu_inline, always_inline)) const DexMethodId *dexGetMethodId(const DexFile *pDexFile, u4 idx) {
    assert(idx < pDexFile->pHeader->methodIdsSize);
    return &pDexFile->pMethodIds[idx];
}

/* return the FieldId with the specified index */
extern inline __attribute__((gnu_inline, always_inline)) const DexFieldId *dexGetFieldId(const DexFile *pDexFile, u4 idx) {
    assert(idx < pDexFile->pHeader->fieldIdsSize);
    return &pDexFile->pFieldIds[idx];
}

/* return the ProtoId with the specified index */
extern inline __attribute__((gnu_inline, always_inline)) const DexProtoId *dexGetProtoId(const DexFile *pDexFile, u4 idx) {
 ...
 */
extern inline __attribute__((gnu_inline, always_inline)) const DexTypeList *dexGetProtoParameters(
...
extern inline __attribute__((gnu_inline, always_inline)) const DexClassDef *dexGetClassDef(const DexFile *pDexFile, u4 idx) {
    assert(idx < pDexFile->pHeader->classDefsSize);
    return &pDexFile->pClassDefs[idx];
}

/* get the interface list for a DexClass */
extern inline __attribute__((gnu_inline, always_inline)) const DexTypeList *dexGetInterfacesList(const DexFile *pDexFile,
...
extern inline __attribute__((gnu_inline, always_inline)) const DexTypeItem *dexGetTypeItem(const DexTypeList *pList,
                                             u4 idx) {
...
extern inline __attribute__((gnu_inline, always_inline)) u4 dexTypeListGetIdx(const DexTypeList *pList, u4 idx) {
    const DexTypeItem *pItem = dexGetTypeItem(pList, idx);
    return pItem->typeIdx;
}

/* get the static values list for a DexClass */
extern inline __attribute__((gnu_inline, always_inline)) const DexEncodedArray *dexGetStaticValuesList(
...
extern inline __attribute__((gnu_inline, always_inline)) const DexAnnotationsDirectoryItem *dexGetAnnotationsDirectoryItem(
    const DexFile *pDexFile, const DexClassDef *pClassDef) {
...
extern inline __attribute__((gnu_inline, always_inline)) const char *dexGetSourceFile(
    const DexFile *pDexFile, const DexClassDef *pClassDef) {
...
extern inline __attribute__((gnu_inline, always_inline)) const DexTry *dexGetTries(const DexCode *pCode) {
...
extern inline __attribute__((gnu_inline, always_inline)) const u1 *dexGetCatchHandlerData(const DexCode *pCode) {
    const DexTry *pTries = dexGetTries(pCode);
    return (const u1 *) &pTries[pCode->triesSize];
}

extern inline __attribute__((gnu_inline, always_inline)) const u1 *dexGetDebugInfoStream(const DexFile *pDexFile,
                                           const DexCode *pCode) {
...
extern inline __attribute__((gnu_inline, always_inline)) const char *dexGetClassDescriptor(const DexFile *pDexFile,
                                             const DexClassDef *pClassDef) {
    return dexStringByTypeIdx(pDexFile, pClassDef->classIdx);
}

/* DexClassDef convenience - get superclass descriptor */
extern inline __attribute__((gnu_inline, always_inline)) const char *dexGetSuperClassDescriptor(const DexFile *pDexFile,
                                                  const DexClassDef *pClassDef) {
...
extern inline __attribute__((gnu_inline, always_inline)) const u1 *dexGetClassData(const DexFile *pDexFile,
                                     const DexClassDef *pClassDef) {
...
extern inline __attribute__((gnu_inline, always_inline)) const DexAnnotationSetItem *dexGetAnnotationSetItem(
    const DexFile *pDexFile, u4 offset) {
    return (const DexAnnotationSetItem *) (pDexFile->baseAddr + offset);
}

/* get the class' annotation set */
extern inline __attribute__((gnu_inline, always_inline)) const DexAnnotationSetItem *dexGetClassAnnotationSet(
...
extern inline __attribute__((gnu_inline, always_inline)) const DexFieldAnnotationsItem *dexGetFieldAnnotations(
    const DexFile *pDexFile, const DexAnnotationsDirectoryItem *pAnnoDir) {
...
extern inline __attribute__((gnu_inline, always_inline)) int dexGetFieldAnnotationsSize(const DexFile *pDexFile,
                                          const DexAnnotationsDirectoryItem *pAnnoDir) {
    return pAnnoDir->fieldsSize;
}

/* return a pointer to the field's annotation set */
extern inline __attribute__((gnu_inline, always_inline)) const DexAnnotationSetItem *dexGetFieldAnnotationSetItem(
...
extern inline __attribute__((gnu_inline, always_inline)) const DexMethodAnnotationsItem *dexGetMethodAnnotations(
    const DexFile *pDexFile, const DexAnnotationsDirectoryItem *pAnnoDir) {
...
extern inline __attribute__((gnu_inline, always_inline)) int dexGetMethodAnnotationsSize(const DexFile *pDexFile,
                                           const DexAnnotationsDirectoryItem *pAnnoDir) {
    return pAnnoDir->methodsSize;
}

/* return a pointer to the method's annotation set */
extern inline __attribute__((gnu_inline, always_inline)) const DexAnnotationSetItem *dexGetMethodAnnotationSetItem(
    const DexFile *pDexFile, const DexMethodAnnotationsItem *pItem) {
    return dexGetAnnotationSetItem(pDexFile, pItem->annotationsOff);
}

/* get the class' parameter annotation list */
extern inline __attribute__((gnu_inline, always_inline)) const DexParameterAnnotationsItem *dexGetParameterAnnotations(
    const DexFile *pDexFile, const DexAnnotationsDirectoryItem *pAnnoDir) {
...
/* get method annotation list size */
extern inline __attribute__((gnu_inline, always_inline)) int dexGetParameterAnnotationsSize(const DexFile *pDexFile,
                                              const DexAnnotationsDirectoryItem *pAnnoDir) {
    return pAnnoDir->parametersSize;
}

/* return the parameter annotation ref list */
extern inline __attribute__((gnu_inline, always_inline)) const DexAnnotationSetRefList *dexGetParameterAnnotationSetRefList(
...
/* get method annotation list size */
extern inline __attribute__((gnu_inline, always_inline)) int dexGetParameterAnnotationSetRefSize(const DexFile *pDexFile,
...
extern inline __attribute__((gnu_inline, always_inline)) const DexAnnotationSetRefItem *dexGetParameterAnnotationSetRef(
...
extern inline __attribute__((gnu_inline, always_inline)) const DexAnnotationSetItem *dexGetSetRefItemItem(
    const DexFile *pDexFile, const DexAnnotationSetRefItem *pItem) {
    return dexGetAnnotationSetItem(pDexFile, pItem->annotationsOff);
}

/* return the Nth annotation offset from a DexAnnotationSetItem */
extern inline __attribute__((gnu_inline, always_inline)) u4 dexGetAnnotationOff(
...
extern inline __attribute__((gnu_inline, always_inline)) const DexAnnotationItem *dexGetAnnotationItem(
 ...
 */
extern inline __attribute__((gnu_inline, always_inline)) u2 dexGetUtf16FromUtf8(const char **pUtf8Ptr) {
...
extern inline __attribute__((gnu_inline, always_inline)) bool dexIsValidMemberNameUtf8(const char **pUtf8Ptr) {
    u1 c = (u1) **pUtf8Ptr;
    if (c <= 0x7f) {
