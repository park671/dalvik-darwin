...
#define ATOMIC_CACHE_CALC \
    ...
    return (Method*) ({
        ...
        u8 firstVersion;
        u8 value;
        hash = (((u8) (thisClass) >> 2) ^ (u8) (methodIdx)) & ((128) - 1);
        pEntry = (methodClassDex->pInterfaceCache)->entries + hash;
        firstVersion = pEntry->version;
        if (pEntry->key1 == (u8) (thisClass) && pEntry->key2 == (u8) (methodIdx)) {
            value = pEntry->value;
            if ((firstVersion & 0x01) != 0 || firstVersion != pEntry->version) {
                if (0)(methodClassDex->pInterfaceCache)->fail++;
                value = (u8) dvmInterpFindInterfaceMethod(thisClass, methodIdx, method, methodClassDex);
            }
            else { if (0)(methodClassDex->pInterfaceCache)->hits++; }
        }
        else {
            value = (u8) dvmInterpFindInterfaceMethod(thisClass, methodIdx, method, methodClassDex);
            dvmUpdateAtomicCache((u8) (thisClass), (u4) (methodIdx), value, pEntry, firstVersion);
        }
        value;
    });
