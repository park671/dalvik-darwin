...
u4 dvmComputeStringHash(StringObject *strObj) {
    ...
    LOGD("[-] chars at %p, chars->contents at %p, offset = 0x%02x, len = 0x%02x\n", chars, (u2 *) chars->contents,
         offset, len);
...
StringObject *dvmCreateStringFromCstrAndLength(const char *utf8Str,
    ...
    dvmSetFieldObject((Object *) newObj, gDvm.offJavaLangString_value,
                      (Object *) chars);
    u1* charInStringObject = ((u1 *) (newObj)) + (gDvm.offJavaLangString_value);
    LOGD("[-] charInStringObject at %p\n", charInStringObject);
    ((JValue *) (charInStringObject))->l = chars;
