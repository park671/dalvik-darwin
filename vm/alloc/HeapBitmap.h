...
#define HB_OFFSET_TO_INDEX(offset_) \
    ((u_int64_t)(offset_) / HB_OBJECT_ALIGNMENT / HB_BITS_PER_WORD)
#define HB_INDEX_TO_OFFSET(index_) \
    ((u_int64_t)(index_) * HB_OBJECT_ALIGNMENT * HB_BITS_PER_WORD)
...
#define HB_OFFSET_TO_MASK(offset_) \
    (1 << \
        (31-(((u_int64_t)(offset_) / HB_OBJECT_ALIGNMENT) % HB_BITS_PER_WORD)))
...
{
    const u_int64_t p = (const u_int64_t)obj;
...
)
{
    ...
    if (obj != NULL) {
        const u_int64_t offset = (u_int64_t)obj - hb->base;
...
#ifndef NDEBUG
    assert(hb->bits != NULL);
    assert((u_int64_t)obj >= hb->base);
    assert(index < hb->bitsLen / sizeof(*hb->bits));
#endif

    if (setBit) {
        if ((u_int64_t)obj > hb->max) {
            hb->max = (u_int64_t)obj;
        }
        if (returnOld) {
