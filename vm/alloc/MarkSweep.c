...
createMarkStack(GcMarkStack *stack)
{
    ...
    stack->limit = limit;
    stack->base = (const Object **)((u_int64_t)limit + size);
...
destroyMarkStack(GcMarkStack *stack)
{
    munmap((char *)stack->limit,
            (u_int64_t)stack->base - (u_int64_t)stack->limit);
    memset(stack, 0, sizeof(*stack));
}
...
//TODO: make sure we're locked
    ...
    if (!setAndReturnMarkBit(ctx, hc)) {
        ...
        if (gMarkParent != NULL) {
            ...
            hc->parent = (const Object *)((u_int64_t)hc->parent | 1);
        }
        hc->markCount++;
#endif
#if WITH_HPROF
...
void dvmHeapMarkRootSet()
{
    ...
    op = refs->table;
    while ((u_int64_t)op < (u_int64_t)refs->nextEntry) {
...
#ifndef NDEBUG
static u_int64_t gLastFinger = 0;
#endif
...
#ifndef NDEBUG
    assert((u_int64_t)finger >= gLastFinger);
    gLastFinger = (u_int64_t)finger;
#endif
...
numEnqueued += schedEnqueue ? 1 : 0;
            ...
            if (schedClear || schedEnqueue) {
                ...
                if (!dvmHeapAddRefToLargeTable(
                        ...
                        (Object *)((u_int64_t)reference | workBits)))
                {
...
sweepBitmapCallback(size_t numPtrs, void **ptrs, const void *finger, void *arg)
{
    ...
    for (i = 0; i < numPtrs; i++) {
         ...
         */
        {
            ...
            obj->clazz = (ClassObject *)((u_int64_t)clazz ^ 0xffffffff);
#if WITH_OBJECT_HEADERS
            *hc = chunk;
#endif
        }
...
{
    return !isMarked(ptr2chunk((u_int64_t)object & ~(HB_OBJECT_ALIGNMENT-1)),
