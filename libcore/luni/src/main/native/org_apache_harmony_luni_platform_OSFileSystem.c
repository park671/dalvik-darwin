...
static jlong harmony_io_readvImpl(JNIEnv *env, jobject thiz, jint fd,
                                  jlongArray jbuffers, jintArray joffsets, jintArray jlengths, jint size) {
    ...
    while (i < size) {
        vectors[i].iov_base = (void *) ((u8) (bufs[i] + offsets[i]));
...
static jlong harmony_io_readDirectImpl(JNIEnv *env, jobject thiz, jint fd,
                                       jlong buf_addr, jint offset, jint nbytes) {
    ssize_t result;
    if (nbytes == 0) {
        return (jlong) 0;
    }

    result = read(fd, (void *) ((jint *) (buf_addr + offset)), (int) nbytes);
    if (result == 0) {
        return (jlong) -1;
    } else {
        return (jlong) result;
...
static jlong harmony_io_writeDirectImpl(JNIEnv *env, jobject thiz, jint fd,
                                        jlong buf, jint offset, jint nbytes) {


    jlong rc = 0;

    /* write will just do the right thing for HYPORT_TTY_OUT and HYPORT_TTY_ERR */
    rc = write(fd, (const void *) ((u8 *) (buf + offset)), (int) nbytes);
...
static JNINativeMethod gMethods[] = {
        ...
        {"readDirectImpl",      "(IJII)J",     (void *) harmony_io_readDirectImpl},
        {"writeDirectImpl",     "(IJII)J",     (void *) harmony_io_writeDirectImpl},
        {"readImpl",            "(I[BII)J",    (void *) harmony_io_readImpl},
        {"writeImpl",           "(I[BII)J",    (void *) harmony_io_writeImpl},
        {"readvImpl",           "(I[J[I[II)J", (void *) harmony_io_readvImpl},
        {"writevImpl",          "(I[J[I[II)J", (void *) harmony_io_writevImpl},
